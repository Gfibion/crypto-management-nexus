...
export const skillsData = {
  ...
  "Software Development": [
    ...
    { name: "Frontend Frameworks", level: 85, icon: "layout", description: "React, Vue, Angular development expertise" },
    { name: "Backend Development", level: 83, icon: "server", description: "Server-side application architecture" },
    { name: "Full-Stack Integration", level: 88, icon: "layers", description: "Complete application stack development" }
  ],
  ...
  "System Architecture": [
    ...
    { name: "Microservices", level: 80, icon: "network", description: "Distributed system architecture design" },
    { name: "Performance Optimization", level: 82, icon: "zap", description: "System performance tuning and scaling" }
  ],
  ...
  "Data & Analytics": [
    ...
    { name: "Data Engineering", level: 78, icon: "database", description: "Data pipeline and ETL processes" },
    { name: "Business Intelligence", level: 82, icon: "chart-line", description: "BI dashboard and reporting systems" },
    { name: "Predictive Analytics", level: 75, icon: "trending-up", description: "Forecasting and trend analysis" },
    { name: "Data Visualization", level: 80, icon: "pie-chart", description: "Interactive charts and dashboards" }
  ],

  "Infrastructure & Security": [
    { name: "Cybersecurity", level: 78, icon: "lock", description: "Security audits and protection strategies" },
    { name: "Network Administration", level: 80, icon: "wifi", description: "Network infrastructure management" },
    { name: "System Administration", level: 82, icon: "server", description: "Server and system maintenance" },
    { name: "Cloud Security", level: 76, icon: "shield-check", description: "Cloud-based security implementation" },
    { name: "Backup & Recovery", level: 85, icon: "hard-drive", description: "Data backup and disaster recovery" },
    { name: "Monitoring & Alerting", level: 78, icon: "activity", description: "System monitoring and alerting" }
  ],
...
export const businessSkills = Object.entries(skillsData)
  .filter(([category]) => ["Strategic Management", "Operations & Leadership", "Financial Management", "Business Development", "Innovation & Entrepreneurship"].includes(category))
...
export const ictSkills = Object.entries(skillsData)
  .filter(([category]) => ["Software Development", "System Architecture", "Data & Analytics", "Infrastructure & Security"].includes(category))
  .flatMap(([category, skills]) => 
    skills.map((skill, index) => ({
      id: `${category.toLowerCase().replace(/\s+/g, '-')}-${index}`,
      name: skill.name,
...
export const consultingSkills = skillsData.Consulting?.map((skill, index) => ({
  id: `consulting-${index}`,
  name: skill.name,
  category: "Consulting",
  proficiency_level: skill.level,
...
export const getCategoryColor = (category: string): string => {
  const colors = {
    "Strategic Management": "blue",
    "Operations & Leadership": "purple", 
    "Financial Management": "green",
    "Business Development": "orange",
    "Innovation & Entrepreneurship": "pink",
    "Software Development": "cyan",
    "System Architecture": "indigo",
